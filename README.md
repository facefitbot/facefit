...
Production-ready проект для Telegram-бота Bella Vladi Face Protocol: AI-анализ лица, финальный PNG-протокол, подробный web-отчет, админ-панель, очереди, воркеры, webhook и нагрузочный mock-тест.
...
cd apps/backend
source .venv/bin/activate
celery -A app.workers.celery_app worker --loglevel=info -Q analysis,report,telegram
```
...
```bash
docker compose up -d --build backend worker_analysis worker_report worker_telegram
```
