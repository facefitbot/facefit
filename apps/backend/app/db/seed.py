    ...
    try:
        ...
        if not db.query(BotSettings).first():
            db.add(
                BotSettings(
                    welcome_text=(
                        "Здравствуйте! Я бот Bella Vladi. Я помогу сделать первичный визуальный "
                        "face-протокол и показать, какие зоны лица можно улучшить с помощью регулярного фейсфитнеса."
                    ),
                    consent_text=(
                    ...
                    photo_instruction_text=(
                        "Пожалуйста, загрузите фото лица: анфас, хорошее освещение, без сильных фильтров, без очков, "
                        "лицо полностью видно, выражение нейтральное."
                    ),
                    waiting_text="Приняла фото. Сейчас формирую первичный face-протокол. Это может занять несколько минут.",
                    after_analysis_text="Ваш face-протокол готов. Ниже отправляю краткую версию и ссылку на подробный отчет.",
