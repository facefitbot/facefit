...
def _shorten_text(text: Any, max_chars: int, fallback: str) -> str:
    cleaned = _clean_text(text) or fallback
    normalized = _replace_zone_label(cleaned)
    ...
    for word in normalized.split():
        next_length = length + len(word) + (1 if words else 0)
        if next_length > max_chars:
            break
        words.append(word)
        length = next_length
    result = " ".join(words).strip(" .,:;—-")
    return result or fallback[:max_chars].strip()
...
def _shape_for_label(label: str, fallback: str = "forehead") -> str:
...
class ProtocolZone(BaseModel):
    ...
    def normalize_label(cls, value: str) -> str:
        return _shorten_text(_replace_zone_label(value), LABEL_LIMIT, "Зона")


class FaceMapCopy(BaseModel):
    title: str = "Face Protocol"
...
def normalize_protocol_slide_copy(protocol_slide_copy: dict[str, Any] | None) -> dict[str, Any]:
    ...
    for index, zone in enumerate(source_zones[:8], start=1):
        if not isinstance(zone, dict):
            continue
        label = _shorten_text(_replace_zone_label(zone.get("label") or zone.get("name")), LABEL_LIMIT, DEFAULT_ZONES[index - 1]["label"])
        shape = zone.get("shape") if zone.get("shape") else _shape_for_label(label, DEFAULT_ZONES[index - 1]["shape"])
        if shape not in {"forehead", "glabella", "eyes", "nasolabial", "cheeks", "jawline", "chin", "neck", "puffiness"}:
            shape = _shape_for_label(label, DEFAULT_ZONES[index - 1]["shape"])
    ...
    if len(normalized_zones) < 8:
        used_shapes = {zone["shape"] for zone in normalized_zones}
    ...
    copy = ProtocolSlideCopy(
        face_map=FaceMapCopy(
            title=_shorten_text(face_map.get("title"), 24, "Face Protocol"),
            subtitle=_shorten_text(face_map.get("subtitle"), 28, "визуальный AI-разбор"),
            main_focus=_limited_list(
                face_map.get("main_focus"),
                3,
                MAIN_FOCUS_LIMIT,
                ["Глаза", "Носогубка", "Овал"],
            ),
        ...
        summary=SummaryCopy(
            ...
            aging_type=_shorten_text(summary.get("aging_type"), SUMMARY_LIMIT, "Усталый тип с акцентом на глаза и овал."),
        ...
        plan=PlanCopy(
            ...
            benefits=_limited_list(plan.get("benefits"), 3, BULLET_LIMIT, ["Более открытый взгляд", "Мягче носогубная зона", "Чётче нижний контур"]),
            forecast=_limited_list(plan.get("forecast"), 3, FORECAST_LIMIT, ["7–14 дней: меньше отёчности", "4–6 недель: свежее лицо", "8–12 недель: устойчивее овал"]),
        ),
    )
...
def build_protocol_slide_copy_from_analysis(analysis_json: dict[str, Any], selected_problems: list[str] | None = None) -> dict[str, Any]:
    ...
    for index, zone in enumerate(source_zones[:8], start=1):
        if not isinstance(zone, dict):
            continue
        label = _replace_zone_label(zone.get("name") or zone.get("label") or DEFAULT_ZONES[index - 1]["label"])
        zones.append(
            {
                "number": zone.get("number") or index,
                "label": label,
                "status": _status_from_zone(zone),
                "shape": _shape_for_label(label, DEFAULT_ZONES[min(index - 1, len(DEFAULT_ZONES) - 1)]["shape"]),
            }
        )

    focus = [
        _replace_zone_label(zone.get("label"))
        for zone in zones
        if zone.get("status") == "priority"
    ]
    focus.extend(_replace_zone_label(zone.get("label")) for zone in zones if zone.get("status") == "attention")
