...
def _web_copy(value: Any, fallback: str = "") -> str:
    ...
    cleanup = {
        ...
        "движение жидкости в тканях": "лёгкость тканей",
        "движения жидкости в тканях": "лёгкости тканей",
        "движению жидкости в тканях": "лёгкости тканей",
        "движение жидкости": "лёгкость тканей",
        "движения жидкости": "лёгкости тканей",
        "движению жидкости": "лёгкости тканей",
    }
...
def _skin_age_sentence(value: Any, fallback: str) -> str:
    ...
    text = re.sub(r",?\s*[Вв]изуально\s*[—–-]\s*на\s*\d+\s*(?:лета|года|год|лет)", "", text).strip(" ,.")
    return text or fallback


def _years_word(value: int) -> str:
    return russian_year_word(value)
...
def _age_plain_blocks(visual_age: int, passport_age: int, zone_focus: str, skin_age: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "text": [
            f"По фото кожа выглядит примерно на {_years_phrase(visual_age)} — {relation}.",
...
def _aging_plain_blocks(
    ...
    if aging_id == "tired_mixed" and len(mixed_components) >= 2:
        combo = _combo_type_phrase(mixed_components)
        face_text = " ".join(_sentence_case(_plain_component_text(component)) for component in mixed_components[:2])
        return {
            "intro": f"У вас сочетание {combo}.",
            "text": [
                (
                    f"Это комбинированный сценарий: в нём сочетаются признаки {combo}. Ниже он разобран последовательно: "
                    "что означает этот тип, как он проявляется на лице и что происходит с кожей."
                ),
            ],
            "chapters": [
                {
                    "title": "Его характеристика",
                    "text": (
                        "Комбинированный тип означает, что лицо стареет не по одной линии. "
                        "Одни зоны быстрее показывают усталость, другие могут давать мягкость тканей, напряжение "
                        "или сухость кожи. Поэтому лицо важно читать по зонам, а не одним общим ярлыком."
                    ),
                },
                {
                    "title": "Что происходит с вашим лицом",
                    "text": (
                        f"По фото особенно важно смотреть на {zones}. {face_text} "
                        "Если поддерживать эти зоны регулярно, лицо выглядит более свежим, открытым и собранным."
                    ),
                },
                {
                    "title": "Что происходит с вашей кожей",
                    "text": (
                        "Кожа и мягкие ткани в смешанном сценарии сильнее реагируют на сон, уход, нагрузку и режим. "
                        "Где-то быстрее появляется сухость, где-то лёгкая припухлость или ощущение усталости, "
                        "поэтому работа должна быть мягкой, регулярной и без перегруза."
                    ),
                },
            ],
        }
    characteristic = _sentence_case(kb.get("characteristic"))
    face_by_type = {
        "muscular": (
            "На лице это чаще видно через лоб, межбровье, жевательную зону и общее выражение. "
            "Мышцы могут держать напряжение даже в покое, поэтому лицо выглядит строже, а мимические линии "
            "становятся заметнее. При этом овал часто долго остаётся чётким, и это хороший ресурс."
        ),
        "deformation_edema": (
            "На лице это чаще видно через зону под глазами, шею, нижнюю треть и овал. "
            "Ткани могут выглядеть тяжелее утром, контур постепенно становится мягче, а лицо быстрее теряет "
            "ощущение лёгкости. Главная задача — поддержать шею, свежесть взгляда и чистую линию овала."
        ),
        "fine_wrinkle": (
            "На лице это чаще видно через текстуру кожи, зону вокруг глаз, щёки и губы. "
            "Контур может оставаться аккуратным, но кожа быстрее показывает сухость, мелкую сетку и усталость. "
            "Здесь особенно важна бережная работа без силового перегруза."
        ),
        "tired_mixed": (
            "На лице это чаще видно через зону под глазами, носослёзную линию, носогубную зону и уголки рта. "
            "Лицо может выглядеть свежим утром, но быстрее уставать к вечеру. Если поддерживать эти зоны, "
            "взгляд раскрывается, а выражение становится спокойнее."
        ),
    }
    skin_by_type = {
        "muscular": (
            "Кожа при мускульном типе часто плотно связана с активной мимикой. Когда мышцы долго держат напряжение, "
            "линии могут становиться глубже и фиксироваться в покое. Поэтому коже важны мягкость, расслабление "
            "и спокойная регулярная работа с мимическими зонами."
        ),
        "deformation_edema": (
            "Кожа и мягкие ткани при деформационно-отечном типе сильнее реагируют на задержку жидкости, сон, соль, "
            "осанку и состояние шеи. Если ткани часто выглядят тяжёлыми, кожа постепенно растягивается, "
            "а нижняя треть хуже держит форму."
        ),
        "fine_wrinkle": (
            "Кожа при мелкоморщинистом типе быстрее просит увлажнения, питания и защиты от пересушивания. "
            "Она может выглядеть тонкой и нежной: это красиво, но требует аккуратности, потому что грубая нагрузка "
            "быстрее подчёркивает сухость и мелкую сетку."
        ),
        "tired_mixed": (
            "Кожа при усталом / смешанном типе часто реагирует снижением сияния и менее ровным тоном к вечеру. "
            "Ей важны мягкая стимуляция питания тканей, спокойный уход и регулярность: тогда лицо выглядит "
            "более отдохнувшим без изменения черт."
        ),
    }
    return {
        "intro": f"Ваш тип старения — {aging_name}.",
        "text": [
            (
                f"Ваш тип старения — {aging_name}. Ниже разбор идёт по порядку: характеристика типа, "
                "что происходит с лицом и как на это реагирует кожа."
            ),
        ],
        "chapters": [
            {"title": "Его характеристика", "text": characteristic},
            {
                "title": "Что происходит с вашим лицом",
                "text": f"{face_by_type.get(aging_id, face_by_type['tired_mixed'])} По фото особенно важно поддерживать {zones}.",
            },
            {"title": "Что происходит с вашей кожей", "text": skin_by_type.get(aging_id, skin_by_type["tired_mixed"])},
        ],
    }
...
def _forecast_plain_blocks(
    ...
    return {
        ...
        "items": items,
        "control": [
            {"title": "Как читать прогноз", "text": "Смотрите не на цифру возраста, а на то, какие зоны первыми начинают влиять на общее впечатление лица."},
            {"title": "Что можно замедлить", "text": "Регулярная мягкая работа помогает дольше сохранять свежесть взгляда, чистоту овала и живое качество кожи."},
            {"title": "Главный ориентир", "text": "Хороший результат — лицо выглядит свежее и собраннее, но остаётся вашим, без ощущения искусственного изменения."},
        ],
    }
...
def _build_detailed_web_sections(
    *,
    ...
    if aging_id == "tired_mixed" and len(mixed_components) >= 2:
        ...
        aging_section_note = "Последовательно: тип, характеристика, что происходит с лицом и кожей."
    else:
...
def build_bella_web_report_data(report: GeneratedReport, settings: BotSettings) -> dict[str, Any]:
    ...
    return {
        ...
        "hero": {
            ...
            "visual_age": _years_phrase(visual_age),
