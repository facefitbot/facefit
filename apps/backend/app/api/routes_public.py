...
function render(r){
  ...
  app.innerHTML=`<main class="sheet">
    ...
    <section class="section"><div class="section-head"><span class="num">08</span><h2 class="section-title">Как лицо может меняться со временем</h2></div><div class="timeline">${(r.forecast||[]).map(i=>`<div class="tl-item"><div class="tl-period">${e(i.period)}</div><div class="tl-card">${e(i.text)}</div></div>`).join("")}</div></section>
...
def public_report_page(token: str, request: Request, db: Session = Depends(get_db)) -> str:
    ...
    causes_html = _items_html(analysis_json.get("causes"), "Тонус мышц, отёчность, осанка и ежедневные мимические привычки.")
    ...
    attention_html = _items_html(skin_type.get("attention_points"), "Отечность, тонус и питание тканей.")
...
<main>
  ...
  <section class="section-grid two">
    ...
    <article class="card">
      <div class="section-head"><span class="number dark">8</span><h2>Как лицо может меняться со временем</h2></div>
      <div class="timeline">
